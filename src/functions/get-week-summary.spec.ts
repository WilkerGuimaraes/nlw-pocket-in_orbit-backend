...
describe('get week week summary', () => {
  it('should be able to get week week summary', async () => {
    const user = await makeUser()

    const weekStartsAt = dayjs(new Date(2025, 9, 10))
      .startOf('week')
      .toDate()
    ...
    expect(result).toEqual({
      summary: {
        ...
        total: 6,
        goalsPerDay: {
          '2025-10-10': expect.arrayContaining([
            expect.objectContaining({ title: 'Ler' }),
          ]),
          '2025-10-08': expect.arrayContaining([
            expect.objectContaining({ title: 'Ler' }),
          ]),
          '2025-10-07': expect.arrayContaining([
